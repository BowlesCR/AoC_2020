...
t_coords = tuple[int, int, int, int]
...
class Grid:
    ...
    def updateBounds(self, coords: t_coords) -> None:
        ...
        self.maxW = max(self.maxW, coords[3])
        self.minW = min(self.minW, coords[3])
            ...
            for y in range(coords[1] - 1, coords[1] + 2):
                for z in range(coords[2] - 1, coords[2] + 2):
                    for w in range(coords[3] - 1, coords[3] + 2):
                        neighbor = (x, y, z, w)
                        if neighbor != coords and self.grid[neighbor] == "#":
                            result += 1
        return result
